...
if modo == "🕐 N citações mais recentes":
    ...
    st.info(
        ...
        "⚠️ Valor aproximado — baseado apenas nas citações registadas nesta BD."
    )
    ...
    tabela_display["Conta para h-index?"] = tabela_display.apply(
        lambda row: "✅ Sim" if row["Citações na BD"] >= row.name else "❌ Não",
        axis=1,
    )
    ...
    st.subheader("📊 Gráfico do H-Index")
